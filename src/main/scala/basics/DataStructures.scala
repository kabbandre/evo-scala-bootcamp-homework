...
object DataStructures {
  ...
  def sortConsideringEqualValues[T](map: Map[T, Int]): List[(Set[T], Int)] = {
    map
      ...
      .transform((_, v) => v.keySet)  // Remove value from value set
      .map(_.swap)                    // Swap key and value
  ...
  val x = sortConsideringEqualValues(input)
  x.map { case (k, _) => k.toList.sorted.mkString("=") }.mkString(" ")
}
